...
app.get('/api/members', async (req, res) => {
  try {
    ...
    if (email) {
      query += ' WHERE LOWER(m.email) = LOWER($1)';
      params.push(email);
    }
...
app.get('/api/enrollments', async (req, res) => {
  const { course_id, member_id, status, date, time } = req.query;

  try {
    let query = `
      SELECT ce.*,
             m.first_name, m.last_name, m.email, m.phone, m.membership_type,
             gc.course_name, gc.location, gc.green_fee, gc.instructor, gc.max_capacity
    ...
    if (course_id) {
      conditions.push(`ce.course_id = $${params.length + 1}`);
      params.push(course_id);
    }

    if (member_id) {
      conditions.push(`ce.member_id = $${params.length + 1}`);
    ...
    if (date) {
      conditions.push(`ce.course_date = $${params.length + 1}`);
      params.push(date);
    }

    if (time) {
      conditions.push(`ce.course_time = $${params.length + 1}`);
      params.push(time);
    }
    ...
    query += ' ORDER BY ce.course_date DESC, ce.course_time DESC';
...
app.post('/api/enrollments', async (req, res) => {
  const { course_id, member_id, course_date, course_time, status = 'Enrolled', notes = null } = req.body;

  if (!course_id || !member_id || !course_date || !course_time) {
    return res.status(400).json({ error: 'Course ID, Member ID, date, and time are required' });
  }

  const validStatuses = ['Enrolled', 'Completed', 'Cancelled', 'Pending'];
  ...
  try {
    ...
    const memberExists = await pool.query('SELECT 1 FROM members WHERE member_id = $1', [member_id]);
    if (memberExists.rowCount === 0) {
      return res.status(400).json({ error: 'Member not found' });
    }

    // Ensure course exists and get capacity
    const courseResult = await pool.query('SELECT max_capacity FROM golf_courses WHERE course_id = $1', [course_id]);
    ...
    // Check if member already enrolled for the same slot
    const duplicateCheck = await pool.query(
      `
        ...
        WHERE course_id = $1
          ...
          AND course_time = $4
      `,
      [course_id, member_id, course_date, course_time]
    );

    if (duplicateCheck.rowCount > 0) {
      return res.status(400).json({ error: 'Member is already enrolled for this course and time slot' });
    }

    // Check capacity for the requested slot
    ...
    if (capacityCheck.rows[0].count >= maxCapacity) {
      return res.status(400).json({ error: 'Course is full for the selected date and time' });
    }

    const enrollment_id = generateId('E');
    const insertQuery = `
      INSERT INTO course_enrollments (
        enrollment_id, course_id, member_id, course_date, course_time, status, notes
      )
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *
    `;
    ...
    const { rows } = await pool.query(
      ...
      `,
      [insertedRows[0].enrollment_id]
    );

    res.status(201).json(rows[0]);
  } catch (err) {
    if (err.code === '23505') {
      return res.status(400).json({ error: 'Enrollment already exists for this slot' });
    }
